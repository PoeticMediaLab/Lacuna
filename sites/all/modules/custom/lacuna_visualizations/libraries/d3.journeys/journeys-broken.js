...
//	console.log(settings.nodes);
	 ...
	 d3.json("./test2.json", function(json) {
		 //console.log(json);
		 for (var i = 0; i < json.length; i++){
			 ...
			 foundUserFlag = false;
			 var visitorName = (stripHtml(json[i].Name));
			 for(var j = 0; j < userNamesColors.length; j++){
				if(userNamesColors[j][0] == visitorName){
					foundUserFlag = true;
					break;
				}
			}
			if(!foundUserFlag){
				userNamesColors.push([visitorName, color(colorsCounter % 20)]);
				selectedUsers.push(visitorName);
				colorsCounter++;
			}
			// userNameColors and selectedUsers initialized.
	...
	// 1. select a list of nodes visited by each user. 
	// 2. for each node past the first, make a link with source =
	// node[i -1], target = node[i]. 
	// 2.5 if link already exists, ignore.
	// 3. fill in other properties of link objects as needed (read: as
	// I get error messages on execution)
  ...
  nodes = settings.nodes;
	...
	// begin displayTooltip
	 function displayTooltip(d) {
		...
		if(d.data.maps_abstract){
			d3.select("#maps-tooltip")
				.select("#abstract")
				.text(d.data.maps_abstract)
				;
			d3.select("#maps-tooltip")
				.select("#abstract")
