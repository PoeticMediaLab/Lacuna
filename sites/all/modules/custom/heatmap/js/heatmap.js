...
var horizontal = false; //switch

//gets the node which contains the content of the article using the path
var articlePath = '//body[1]/div[1]/div[1]/div[1]/article[1]/div[1]/div[1]/div[1]/div[1]';
var pageContainer = document.evaluate(articlePath, document, null, XPathResult.UNORDERED_NODE_ITERATOR_TYPE, null);
var articleNode = pageContainer.iterateNext(); //article node = node where content starts

//initializes array representing the density of annotations in each increment
var aDen = [];

//establishes the increment as an integer that is ~ the length of the text divided into "width" increments
var increment = incrementScale*Math.round(articleNode.textContent.length/width) + extra;

var currentNode = articleNode.childNodes[0]; //starts with the first child of the article
articleNode.classList.add("checked");
var lCount = 0; //represents how much text has been included so far, initialized to 0
var anCount = 0; //represents the number of annotations included so far, initialized to 0
