...
(function() {
  ...
  Annotator.Plugin.Filters = (function(superClass) {
    ...
    function Filters(element, options) {
      ...
      this.filterSelected = bind(this.filterSelected, this);
      var filter, i, item, j, len, len1, pair, query, ref, ref1;
    ...
    Filters.prototype.pluginInit = function() {
      this.annotator.subscribe("annotationViewerShown", (function(_this) {
    ...
    Filters.prototype.setup = function(annotations) {
      var annotation, highlight, i, j, len, len1, ref;
    ...
    Filters.prototype.filterSelected = function(event, ui) {
      ...
      if (!this.Model.filterIsActive(filter, value)) {
        if (filter === 'category') {
          this.View.checkboxDisable('highlights');
        }
    ...
    Filters.prototype.buttonClick = function(event) {
      ...
      if (previous.attr('id') === active.attr('id')) {
        return;
      }
  ...
  Model = (function() {
        ...
        results.push((function() {
          ...
          for (filter in this.state.filters) {
            if (annotation[filter] != null) {
              switch (filter) {
                case 'user':
                  results1.push(this.addFilterValue(filter, annotation['user'].name));
                  break;
                case 'tags':
                  ...
                  break;
                default:
                  results1.push(this.addFilterValue(filter, annotation[filter]));
              }
            } else {
    ...
    Model.prototype.addFilterValue = function(filter, value) {
      if (indexOf.call(this.state.filters[filter].values, value) < 0) {
    ...
    Model.prototype.filterIsActive = function(filter, value) {
      if (!value) {
        return Object.keys(this.state.filters[filter].active).length > 0;
      }
    ...
    Model.prototype.dropHidden = function(annotations) {
      var annotation, i, len, ref, shown;
      shown = [];
      for (i = 0, len = annotations.length; i < len; i++) {
        annotation = annotations[i];
        if (ref = annotation.id, indexOf.call(this.state.ids.hidden, ref) < 0) {
      ...
      for (filter in this.state.filters) {
        results.push((function() {
          ...
          for (value in this.state.filters[filter].active) {
            results1.push(this.removeFilter(filter, value));
          }
          return results1;
        }).call(this));
      }
    ...
    Model.prototype.union = function(arrays) {
      var arr, i, id, j, len, len1, result;
      result = [];
      for (i = 0, len = arrays.length; i < len; i++) {
        arr = arrays[i];
        for (j = 0, len1 = arr.length; j < len1; j++) {
          id = arr[j];
          if (indexOf.call(result, id) < 0) {
            result.push(id);
          }
        }
      }
    ...
    Model.prototype.computeFilters = function() {
      var arrays, filter, i, id, ids, len, ref, value;
      ...
      ref = this.state.ids.all;
      for (i = 0, len = ref.length; i < len; i++) {
        id = ref[i];
        if (indexOf.call(this.state.ids.hidden, id) < 0) {
      ...
      if (filter === 'none') {
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          if (currentValue instanceof Array) {
            if (indexOf.call(currentValue, value) < 0) {
  ...
  View = (function() {
    function View() {
      this.viewerShown = bind(this.viewerShown, this);
      this.setup = bind(this.setup, this);
    }

    View.prototype.setup = function(Controller, Model) {
      var filter, ref, title, values;
      ...
      if (Drupal.PDFDocumentView) {
        Drupal.PDFDocumentView.loaded.then((function(_this) {
          return function() {
            return _this.document = Drupal.PDFDocumentView.frameDocument;
          };
        })(this));
      } else {
    ...
    View.prototype.showAnnotations = function(ids) {
      ...
      for (i = 0, len = ids.length; i < len; i++) {
        id = ids[i];
        $('.' + select.annotation + id, this.document).removeClass(select.hide);
        this.Model.annotation(id)._isHidden = false;
      }
    ...
    View.prototype.hideAnnotations = function(ids) {
      ...
      for (i = 0, len = ids.length; i < len; i++) {
        id = ids[i];
        $('.' + select.annotation + id, this.document).addClass(select.hide);
        this.Model.annotation(id)._isHidden = true;
      }
      return $(document).trigger('annotation-filters-changed');
    };
    ...
    View.prototype.scrollTo = function(annotation) {
      ...
      if (annotation.pdfRange) {
        Drupal.PDFDocumentView.pagesLoaded.then((function(_this) {
          return function() {
            return _this.scrollToPDFHighlight(annotation);
          };
        })(this));
        return;
      }
    ...
    View.prototype.scrollToPDFHighlight = function(annotation) {
      var pageNumber;
      pageNumber = annotation.pdfRange.pageNumber;
      return Drupal.PDFDocumentView.scrollToPage(pageNumber).then((function(_this) {
        return function() {
          var $highlight, bottom, left, ref, right, top;
          $highlight = annotation.$pdfHighlight;
          ref = $highlight[0].getBoundingClientRect(), top = ref.top, left = ref.left, bottom = ref.bottom, right = ref.right;
          top = (top + bottom) / 2;
          left = (left + right) / 2;
          return $(_this.element).annotator().annotator('showViewer', [annotation], {
            top: top,
            left: left
          });
        };
