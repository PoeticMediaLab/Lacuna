...
! function () {
    ...
    // Zhila's code here. Still working on this. 
    // MLW - Let's move this into a separate file, not in Annotator core
    Annotator.Plugin.Categories = function (_super) {
        __extends(Categories, _super);

        function Categories() {
            this.setAnnotationCategories = __bind(this.setAnnotationCategories, this);
            this.updateField = __bind(this.updateField, this);
            return Categories.__super__.constructor.apply(this, arguments)
        }
        Categories.prototype.options = {
            parseCategories: function (string) {
                var categories;
                string = $.trim(string);
                categories = [];
                if (string) {
                    categories = string.split(/\s+/)
                }
                return categories
            },
            stringifyCategories: function (array) {
                return array.join(" ")
            }
        };
        ...
        Categories.prototype.pluginInit = function () {
            ...
            this.field = this.annotator.editor.addField({
                label: Annotator._t("Add some categories here") + "…",
                load: this.updateField,
                submit: this.setAnnotationCategories
            });
            this.annotator.viewer.addField({
                load: this.updateViewer
            });
            if (this.annotator.plugins.Filter) {
                this.annotator.plugins.Filter.addFilter({
                    ...
                    property: "categories",
                    isFiltered: Annotator.Plugin.Category.filterCallback
                })
            }
            return this.input = $(this.field).find(":input")
        };
        Categories.prototype.parseCategories = function (string) {
            return this.options.parseCategories(string)
        };
        Categories.prototype.stringifyCategories = function (array) {
            return this.options.stringifyCategories(array)
        };
        Categories.prototype.updateField = function (field, annotation) {
            var value;
            value = "";
            if (annotation.categories) {
                value = this.stringifyCategories(annotation.categories)
            }
            return this.input.val(value)
        };
        Categories.prototype.setAnnotationCategories = function (field, annotation) {
            return annotation.categories = this.parseCategories(this.input.val())
        };
        Categories.prototype.updateViewer = function (field, annotation) {
            field = $(field);
            if (annotation.categories && $.isArray(annotation.categories) && annotation.categories.length) {
                return field.addClass("annotator-categories").html(function () {
                    var string;
                    return string = $.map(annotation.categories, function (category) {
                        return '<span class="annotator-category">' + Annotator.Util.escape(category) + "</span>"
                    }).join(" ")
                })
            } else {
                return field.remove()
    ...
    Annotator.Plugin.Categories.filterCallback = function (input, categories) {
        var keyword, keywords, matches, category, _k, _l, _len2, _len3;
        if (categories == null) {
            categories = []
        }
        matches = 0;
    ...
    Annotator.prototype.setupPlugins = function (config, options) {
        ...
        plugins = ["Unsupported", "Auth", "Tags", "Filter", "Store", "AnnotateItPermissions", "Categories"];
        if (win.Showdown) {
